...
type ('a, 's) t =
  ...
  ; mutable destroyed_at_fork : (('a, 's) instruction * Reg.Set.t) list
...
let add_superpressure_regs
    (type a s)
    t
    (module Proc : Proc_intf.S with type addressing_mode = a and type specific_operation = s)
    op live_regs res_regs spilled
  =
...
let rec reload 
  ...
  fun
    (type a s)
    t
    ((module Proc : Proc_intf.S with type addressing_mode = a and type specific_operation = s) as proc)
    (i : (a,s) instruction)
    before
  ->
  ...
  | Iop(Icall_ind | Icall_imm _ | Iextcall { alloc = true; }) ->
      ...
      let (new_next, finally) = reload t proc i.next i.live in
  ...
  | Iop op ->
      let new_before =
        ...
        then before
        else add_superpressure_regs t proc op i.live i.res before in
      let after =
        Reg.diff_set_array (Reg.diff_set_array new_before i.arg) i.res in
      let (new_next, finally) = reload t proc i.next after in
      (add_reloads t
         (Reg.inter_set_array new_before i.arg)
         (instr_cons_debug i.desc i.arg i.res i.dbg new_next),
       finally)
  | Iifthenelse(test, ifso, ifnot) ->
      let at_fork = Reg.diff_set_array before i.arg in
      let date_fork = t.current_date in
      let (new_ifso, after_ifso) = reload t proc ifso at_fork in
      let date_ifso = t.current_date in
      t.current_date <- date_fork;
      let (new_ifnot, after_ifnot) = reload t proc ifnot at_fork in
      t.current_date <- Int.max date_ifso t.current_date;
      let (new_next, finally) =
        reload t proc i.next (Reg.Set.union after_ifso after_ifnot) in
      let new_i =
  ...
  | Iswitch(index, cases) ->
      ...
      let new_cases =
        Array.map
          (fun c ->
            t.current_date <- date_fork;
            let (new_c, after_c) = reload t proc c at_fork in
            after_cases := Reg.Set.union !after_cases after_c;
            date_join := Int.max !date_join t.current_date;
            new_c)
          cases in
      t.current_date <- !date_join;
      let (new_next, finally) = reload t proc i.next !after_cases in
      (add_reloads t
         (Reg.inter_set_array before i.arg)
                   (instr_cons (Iswitch(index, new_cases))
                               i.arg i.res new_next),
       finally)
  | Icatch(rec_flag, handlers, body) ->
      let (new_body, after_body) = reload t proc body before in
      let rec fixpoint () =
        let at_exits =
      ...
      let union = List.fold_left
          ...
          after_body res in
      let (new_next, finally) = reload t proc i.next union in
  ...
  | Itrywith(body, handler) ->
      ...
      let (new_handler, after_handler) = reload t proc handler before_handler in
      let (new_next, finally) =
        reload t proc i.next (Reg.Set.union after_body after_handler) in
...
let rec spill
  ...
  fun
    (type a s)
    t
    ((module Platform : Platform_intf.S with type Arch.addressing_mode = a and type Arch.specific_operation = s) as platform)
    (i : (a,s) instruction)
    finally
  ->
  match i.desc with
  ...
  | Ireturn | Iop(Itailcall_ind) | Iop(Itailcall_imm _) ->
      (i, Reg.Set.empty)
  | Iop Ireload ->
      let (new_next, after) = spill t platform i.next finally in
  ...
  | Iop op ->
      ...
      let before1 = Reg.diff_set_array after i.res in
      let before =
        if operation_can_raise Platform.Arch.operation_can_raise op
      ...
      (instr_cons_debug i.desc i.arg i.res i.dbg
       ...
       before)
  | Iifthenelse(test, ifso, ifnot) ->
      let (new_next, at_join) = spill t platform i.next finally in
      let (new_ifso, before_ifso) = spill t platform ifso at_join in
      let (new_ifnot, before_ifnot) = spill t platform ifnot at_join in
      if
  ...
  | Iswitch(index, cases) ->
      ...
      let new_cases =
        Array.map
          (fun c ->
            let (new_c, before_c) = spill t platform c at_join in
  ...
  | Icatch(rec_flag, handlers, body) ->
      ...
      let rec fixpoint () =
        let res =
          List.map (fun (_, handler) -> spill t platform handler at_join) handlers in
      ...
      let (new_body, before) = spill t platform body at_join in
      let new_handlers = List.map2
          (fun (nfail, _) (new_handler, _) -> (nfail, new_handler))
          handlers res in
  ...
  | Itrywith(body, handler) ->
      let (new_next, at_join) = spill t platform i.next finally in
      let (new_handler, before_handler) = spill t platform handler at_join in
      let saved_spill_at_raise = !spill_at_raise in
      spill_at_raise := before_handler;
      let (new_body, before_body) = spill t platform body at_join in
...
let fundecl
  ...
  =
  let t = create () in
  let (body1, _) = reload t (module Platform.Proc) f.fun_body Reg.Set.empty in
  let (body2, tospill_at_entry) = spill t platform body1 Reg.Set.empty in
  let new_body =
